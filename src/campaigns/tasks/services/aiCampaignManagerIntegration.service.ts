...
export class AiCampaignManagerIntegrationService extends createPrismaBase(
  ...
  private parseCampaignPlanToTasks(
    ...
    if (campaignPlanJson.tasks?.all_tasks?.length > 0) {
      campaignPlanJson.tasks.all_tasks.forEach((task) => {
        tasks.push(this.convertJsonTaskToCampaignTask(task, campaign))
      })
    } else {
      this.logger.warn(
  ...
  private convertJsonTaskToCampaignTask(
    ...
    _campaign: CampaignWithPathToVictory,
  ): CampaignTask {
    return {
  ...
  private mapToFlowType(type: unknown): CampaignTaskType {
    if (typeof type === 'string') {
      ...
      switch (normalizedType) {
        ...
        case 'external':
        case 'link':
          return CampaignTaskType.externalLink
        case 'general':
        case 'learn':
        case 'training':
          return CampaignTaskType.general
        default:
          return CampaignTaskType.general
      }
    }
  ...
  private createDefaultTasks(_campaignId: number): CampaignTask[] {
    return [
      {
