...
const userCleanupClerkThrottle = <T>(fn: () => Promise<T>) =>
  throttle({
    // max 2 req/s for user cleanup
    limit: 2,
    interval: 1000,
  })(fn)()
...
export class UsersService extends createPrismaBase(MODELS.User) {
  ...
  async deleteTestUsers() {
    try {
      ...
      const { data: clerkUsers } = await userCleanupClerkThrottle(() =>
      ...
      for (const clerkUser of clerkUsers) {
        if (clerkUser.createdAt < cutoff.getTime()) {
