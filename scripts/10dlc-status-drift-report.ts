/**
 * 10DLC Compliance Mismatch Report
 *
 ...
 *
 * Usage:
 *   npx tsx scripts/10dlc-compliance-mismatch-report.ts [--limit=N]
 *
 * Options:
...
const EXCLUDED_CAMPAIGN_IDS = [
  151557, // samhughes@duck.com
  20, // zaktomich@gmail.com
  962, // dllane2012@gmail.com
  27835, // lurchbulldog@gmail.com
  117404, // tevynbell+test10dlc@gmail.com
  3078, // lurchbulldog+prod@gmail.com
  98823, // aevans@voteandraeevans.com
]
...
// HubSpot property name — see hubspot-10dlc-status.ts
...
// workflows (see HUBSPOT_INTEGRATION.md).
//
...
async function main() {
  ...
  const withoutId = new Set(
    dbRows.filter((r) => !r.hubspot_id).map((r) => r.email.toLowerCase()),
  ).size
  console.log(
    `  ${uniqueHubSpotIds.length} with hubspotId, ${withoutId} missing hubspotId`,
  )
  ...
  const csvPath = `scripts/10dlc-compliance-mismatch-report-${timestamp}.csv`
