...
- **Duplicated implementation work** — every infra or tooling change must be done N times: implemented, reviewed, and deployed in each repo independently. 30+ PRs in 6 months were purely duplicative cross-repo work.
- **Coordination overhead** — cross-service releases require manual sequencing across repos. Changes that should be atomic commits instead require sequential PRs with separate review and deploy cycles.
- **Accelerating version drift** — dependency versions have already diverged across repos, making future changes harder and riskier. A fix that works in one repo may not work in another.
- **Compounding growth** — each new service multiplies all of the above linearly. We lack the ops tooling to manage this at scale, and building it is investment that works against shipping product.
...
### Duplicated implementation work: ~30 PRs in 6 months
...
A code-level audit confirms this isn't incidental — the duplication is structural:
...
### Coordination overhead: sequential PRs and manual releases

Cross-service changes require sequential PRs across repos instead of atomic commits. On Feb 2 and Feb 12, we had "prod deploy" PRs merged on the same day across multiple repos — suggesting releases that required touching multiple repos in sequence.

The deployment circuit breaker fix is the clearest example: a production-impacting deployment bug required 3 separate PRs on the same day. The people-api and election-api PRs literally just say "See gp-api #1230." A bug that should have been a one-line fix in one place required three PRs, three reviews, and three deployments — with three chances to miss one.

With Dependabot now active in all three repos, every dependency update generates 3 separate PRs to review and merge. Over the course of a year, this will add up to hundreds of redundant Dependabot PRs.

### Accelerating version drift
...
This drift has real consequences: a security patch or breaking change in any of these dependencies must now be evaluated and applied independently in each repo. The longer we wait, the more divergent the repos become and the harder it is to bring them back in line.

### Compounding growth: each new service multiplies the problem

Each new service multiplies all of the above linearly. We currently lack the ops tooling (release trains, cross-repo CI, service mesh, contract testing) that organizations typically use to manage multi-service deployments safely. This isn't a criticism — building that tooling is a significant investment that we should only make if the multi-service architecture is providing commensurate value.
