...
- APM pricing is *per host*, on top of infrastructure pricing
...
## 4. AWS Managed Grafana (AMG)

### What AMG Is (and Isn't)

AMG is **only the Grafana visualization/dashboarding layer** — it does NOT include managed backends for metrics, logs, or traces. You must assemble separate AWS services for each signal type:

| Signal | AWS Backend | Notes |
|--------|-------------|-------|
| Metrics | Amazon Managed Prometheus (AMP) | PromQL compatible |
| Logs | CloudWatch Logs | Proprietary query language (Logs Insights), no LogQL |
| Traces | AWS X-Ray | Proprietary, not OTel-native on the backend |
| RUM/Browser | CloudWatch RUM | Basic Core Web Vitals and error tracking |

There is **no managed Loki, Mimir, or Tempo from AWS**. If you want those, you must self-host them on EKS/EC2.

### Pricing Model

#### AMG Workspace (Per User)

| License Type | Cost |
|--------------|------|
| Editor / Admin | $9/active user/workspace/month |
| Viewer | $5/active user/workspace/month |
| Enterprise Plugins add-on | +$45/active user/workspace/month |

No workspace fee, no upfront costs. Minimum 1 editor per workspace.

#### Amazon Managed Prometheus (AMP)

| Component | Price |
|-----------|-------|
| Ingestion (first 2B samples/month) | $0.90 per 10M samples |
| Ingestion (next 250B samples) | $0.35 per 10M samples |
...
| Traces retrieved/scanned | $0.50 per 1M traces |
...
| Component | Price |
|-----------|-------|
| RUM events | $1.00 per 100k events |

### Estimated Monthly Cost

| Component | Calculation | Estimate |
|-----------|-------------|----------|
| AMG licenses | 15 editors × $9 | $135 |
| CloudWatch Logs (~24 GB/mo) | 24 GB × $0.50 ingest + storage | ~$13 |
| AMP (~35 GB/mo metrics) | Highly dependent on sample count (see caveat) | ~$90-$800 |
| X-Ray (~78 GB/mo traces+APM) | ~81M traces × $5/1M + queries | ~$425 |
| CloudWatch RUM (~23M events/mo) | 23M × $1/100k | ~$229 |
| **Total** | | **~$892-$1,602/month** |

> **Caveat**: AMP pricing depends on metric sample count, not GB. The range is wide because 35 GB/month of metrics could be anywhere from 1B to 19B samples depending on cardinality and scrape interval. At lower cardinality (~1-2B samples), AMP costs ~$90-180. At high cardinality, it can exceed $700. You'd need to measure actual sample rates. A reasonable mid-range estimate puts the total at **~$1,000-$1,200/month**.

### Features

| Feature | Support | Notes |
|---------|---------|-------|
| APM | ❌ No real APM | No Application Observability. X-Ray provides basic service maps and trace analysis, but no RED metrics, anomaly detection, or guided root cause analysis. |
| Logging | ⚠️ Limited | CloudWatch Logs only — proprietary query language (Logs Insights), no LogQL. Can self-host Loki and connect it, but then you're managing infrastructure. |
| RUM / Browser Monitoring | ⚠️ Basic | CloudWatch RUM covers Core Web Vitals and JS errors. No session replay, no deep frontend-to-backend trace correlation like Grafana Cloud's Frontend Observability. |
| OpenTelemetry | ✅ Via ADOT | AWS Distro for OpenTelemetry (ADOT) — a supported OTel Collector distribution. However, backends (X-Ray, CloudWatch) are proprietary, not OTel-native. |
| Alerting | ✅ Full | Standard Grafana Alerting — same as OSS. Contact points, notification policies, silences. |
| Incident Management | ❌ None | No built-in incident management. Must integrate PagerDuty, OpsGenie, or AWS Systems Manager Incident Manager externally. |
| Terraform | ✅ Official | AWS provider for workspace management + Grafana provider for dashboards/alerts. Both mature. |
| Pulumi | ✅ Official | AWS Pulumi provider supports AMG workspaces. Grafana provider for dashboard management. |
| AI / MCP | ❌ None | No AI assistant, no MCP server. Amazon Q is a separate product, not integrated into AMG. |
| Tracing | ⚠️ X-Ray only | X-Ray provides service maps and trace timelines. Proprietary format. Can also connect to self-hosted Tempo/Jaeger. |

### Critical Issue: Version Lag

AMG currently runs **Grafana 10.4**. Grafana Cloud runs **Grafana 11.x/12.x**. AWS has acknowledged the lag but as of February 2026, Grafana 11+ has still not shipped for AMG. This means:

- Missing all Grafana 11 and 12 features (new visualizations, improved alerting, Explore improvements, performance gains)
- Some community plugins requiring Grafana 11+ won't work
- Security support window for 10.4 is a concern

### Implementation Complexity: Medium-High

- AMG workspace setup is straightforward, but the **multi-service architecture** adds complexity
- You must provision and wire together 4-5 separate AWS services (AMG + AMP + CloudWatch + X-Ray + ADOT)
- Authentication requires AWS IAM Identity Center (SSO) or SAML — no simple username/password
- No pre-built APM dashboards — you build everything yourself
- **Grafana Cloud comparison**: In Grafana Cloud, you get one API key and one OTLP endpoint that handles all signals. In AMG, each signal goes to a different backend with different configuration.
- **Vendor lock-in**: X-Ray and CloudWatch are proprietary. AMP (Prometheus) is the most portable component. Using ADOT/OTel for instrumentation preserves portability at the collection layer.
- **Best fit**: Teams already deep in AWS that want Grafana dashboards on top of existing CloudWatch/X-Ray data, with data sovereignty requirements
...
| **AWS Managed Grafana** | **~$1,000-$1,200** | Medium — AMP sample-based pricing is the wildcard |
| **Self-hosted LGTM** | **~$5,930-$9,160** | Medium — infrastructure is predictable, but engineering time is the hidden cost |

### Feature Parity

| Requirement | Grafana Cloud | Datadog | AWS Managed Grafana | Self-hosted LGTM |
|-------------|:---:|:---:|:---:|:---:|
| General APM | ✅ | ✅ Best-in-class | ❌ | ❌ |
| Logging (slice & dice, dashboards) | ✅ | ✅ | ⚠️ CloudWatch only | ✅ |
| Browser Monitoring (RUM) | ✅ | ✅ | ⚠️ Basic (CW RUM) | ⚠️ Partial |
| OpenTelemetry (minimal custom agents) | ✅ First-class | ⚠️ Pushes dd-agent | ⚠️ ADOT, but proprietary backends | ✅ |
| Alerting | ✅ | ✅ | ✅ | ✅ |
| Incident Management | ✅ | ✅ (separate SKUs) | ❌ | ⚠️ Degrading |
| Terraform IaC | ✅ Official | ✅ Official | ✅ Official | ✅ Official |
| Pulumi IaC | ⚠️ Community | ✅ Official | ✅ Official | ⚠️ Community |
| AI / MCP | ✅ Strong | ✅ Strong | ❌ | ❌ |
| Tracing | ✅ | ✅ | ⚠️ X-Ray only | ✅ |
...
| **AWS Managed Grafana** | Medium-High | Weeks | Low (managed UI), but you manage multiple backend services |
...
1. **Cost**: At ~$344/month, it's roughly **3.5x cheaper than Datadog**, **3x cheaper than AMG**, and **17-27x cheaper than self-hosting** (when factoring in engineering time)
...
4. **Simplicity**: Fully managed with low implementation complexity — one OTLP endpoint for all signals
...
### Why not AWS Managed Grafana:

- **~3x more expensive** than Grafana Cloud, primarily driven by X-Ray and AMP costs
- AMG is just the visualization layer — you must assemble and wire together 4-5 separate AWS services (AMG + AMP + CloudWatch Logs + X-Ray + ADOT + CloudWatch RUM)
- **No APM, no incident management, no AI features** — significant feature gaps vs Grafana Cloud
- **Grafana version lag**: AMG is stuck on Grafana 10.4 while Grafana Cloud runs 11.x/12.x. AWS has not shipped an update despite acknowledging the problem.
- Proprietary backends (X-Ray, CloudWatch) create AWS lock-in on the data layer, even though ADOT keeps the instrumentation layer portable
- Only makes sense if you have hard data sovereignty/compliance requirements mandating data stays in your AWS account
